...
def encrypt_binary(message_bin, qk, encrypter=Rabbit):
    ...
    return encrypter(enc_long(sg[0])).encrypt_bin(message_bin), kg
...
def decrypt_binary(message, kg, dk, decrypter=Rabbit):
    '''Decrypt a message using temp. public key kg and private key dk'''
    bits, d = dk
    # print(message)
    try:
    ...
    sg = mulp(cp, cq, cn, kg, d)        # shared secret d*(k*G) = k*d*G
    return decrypter(enc_long(sg[0])).decrypt_bin(message)
