...
import logging
import math
sys.path.append(os.path.abspath(os.path.join('app','process','cryptography','ntru1Enc','ntru1')))
# from docopt import docopt
from ntru1.ntrucipher import NtruCipher
from ntru1.mathutils import random_poly
...
def encryptFile(pub_key_file, input_arr, bin_output=False, block=False):
    pub_key = np.load(pub_key_file, allow_pickle=True)
    ntru = NtruCipher(int(pub_key['N']), int(pub_key['p']), int(pub_key['q']))
    ntru.h_poly = Poly(pub_key['h'].astype(np.int)[::-1], x).set_domain(ZZ)
    if not block:
...
def decryptFile(priv_key_file, input_arr, bin_input=False, block=False):
    priv_key = np.load(priv_key_file, allow_pickle=True)
    ntru = NtruCipher(int(priv_key['N']), int(priv_key['p']), int(priv_key['q']))
    ntru.f_poly = Poly(priv_key['f'].astype(np.int)[::-1], x).set_domain(ZZ)
    ntru.f_p_poly = Poly(priv_key['f_p'].astype(np.int)[::-1], x).set_domain(ZZ)
...
# p = 3
# q = 32
# with open("C://test.txt", 'rb') as file:
#     input = file.read()
...
# print(output)
# input_arr = output.reshape((-1, ntru.N))
# # print(input_arr)
# output = np.array([])
# block_count = input_arr.shape[0]
# for i, b in enumerate(input_arr, start=1):
...
#     output = np.concatenate((output, next_output))
# print(output)
# result = padding_decode(output, ntru.N)
# print(result)
# k = int(math.log2(ntru.q))
# pad = k - len(input) % k
# if pad == k:
#     pad = 0
# input_arr = np.array([int("".join(n.astype(str)), 2) for n in
#                               np.pad(np.array(input), (0, pad), 'constant').reshape((-1, k))])

# np.savez_compressed("C://pubkey", N=N, p=p, q=q, h=h)
# pub_key = np.load("C://pubkey.npz", allow_pickle=True)
# ntru = NtruCipher(int(pub_key['N']), int(pub_key['p']), int(pub_key['q']))
# ntru.h_poly = Poly(pub_key['h'].astype(int)[::-1], x).set_domain(ZZ)
# ntru = NtruCipher(int(N), int(p), int(q))
# ntru.h_poly = Poly(h.astype(int)[::-1], x).set_domain(ZZ)
# print (ntru.h_poly)

# np.savez_compressed("C://privkey", N=N, p=p, q=q, f=f, f_p=f_p)
# priv_key = np.load("C://privkey.npz", allow_pickle=True)
# ntru = NtruCipher(int(priv_key['N']), int(priv_key['p']), int(priv_key['q']))
