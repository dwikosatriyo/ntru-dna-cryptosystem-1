...
class NtruCipher:
    ...
    def generate_random_poly(self):
        random_polynomial = random_poly(self.N, int(math.sqrt(self.q)))
    ...
    def generate_public_key(self, f_poly, g_poly):
        self.f_poly = f_poly
        self.g_poly = g_poly
        # print("Trying to invert: {}".format(self.f_poly))
    ...
    def encryptMessage(self, msg):
        msg = Poly(msg[::-1], x).set_domain(ZZ)
        rand_poly = self.generate_random_poly()
        log.info("r: {}".format(rand_poly))
        log.info("r coeffs: {}".format(Counter(rand_poly.coeffs())))
        log.info("msg: {}".format(msg))
        log.info("h: {}".format(self.h_poly))
        return (((rand_poly * self.h_poly).trunc(self.q) + msg) % self.R_poly).trunc(self.q).all_coeffs()[::-1]

    def decryptMessage(self, msg):
        msg = Poly(msg[::-1], x).set_domain(ZZ)
        log.info("f: {}".format(self.f_poly))
        log.info("f_p: {}".format(self.f_p_poly))
        a_poly = ((self.f_poly * msg) % self.R_poly).trunc(self.q)
        log.info("a: {}".format(a_poly))
        b_poly = a_poly.trunc(self.p)
        log.info("b: {}".format(b_poly))
        return ((self.f_p_poly * b_poly) % self.R_poly).trunc(self.p)
