...
class NTRUParams(object):
	def __init__(self, k, choice="speed"): # NTRU Paramgen Paper page 16
		if k==112:
			...
			elif choice == "hoffstein":
				self.N, self.d_f = (401, 101)
		elif k==128:
			if choice == "space":
...
def strToBin(m):
	...
	# 	ord_str = [ord(l) for l in m]
	# else:
	# 	ord_str = [int(x) for x in m]
	# return "".join([bin(c)[2:].zfill(8) for c in ord_str])

def binToStr(b):
	# bs = []
	# for i in range(int(len(b)/8)):
	# 	bs.append(chr(int("".join([ str(x) for x in b[i*8:(i+1)*8] ]),2)))
	return "".join([chr(int("".join([ str(x) for x in b[i*8:(i+1)*8] ]),2)) for i in range(int(len(b)/8))])
	# return "".join(bs)
...
def NTRUEncrypt(ring, pub, m):
	...
	m = list(map(int,list(strToBin(m))))
	if len(m) > ring.N:
		msplit = [ m for m in chunk(ring.N, m) ]
		n = len(msplit[-1])
		m = [ConvPoly(m, ring.N) for m in msplit]
	else:
		n = len(m)
		m = [ConvPoly(m, ring.N)]
	
	menc = [NTRUBlockEncrypt(ring, pub, m) for m in m]
...
def NTRUDecrypt(key, cl, n):
	...
	cl = [NTRUBlockDecrypt(key, c) for c in cl]
...
def NTRUEncryptBinary(ring, pub, m):
...
def NTRUDecryptBinary(key, cl, n):
	if len(cl)==1:
		m = NTRUBlockDecrypt(key, cl[0])
		return m.coef[:n]
	cl = [NTRUBlockDecrypt(key, c) for c in cl]
